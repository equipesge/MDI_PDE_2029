...
class SaidaDados:
    ...
    def imprimeLog(self, tempo_inicial):
        ...
        # escreve o usuario que fez o teste pela ultima vez
        ...
        saidaResul.write("Versao ModeloJorge: " + vModeloJorge + "\n");
            ...
            for isis in (m.subsistemas):
                for ipat in (m.patamares):
                    for iper in (m.periodosTotal):
                        ...
                        saidaResul.write(str(self.sin.subsistemas[isis].montanteRenovExUFV[iper] * self.sin.subsistemas[isis].fatorPatUFV[ipat][iper%12]) + ";");
    ...
    def imprimeSaidaExpansao(self):        
        ...
        # impressao de hidro
        saidaResul = open(self.caminho + "saidaHidro.txt", "w");

        for hidro in modelo.projUHENova:
            nmeses = sum(sum(modelo.investHidro[hidro,iperaux].value for iperaux in range(0,iper)) for iper in modelo.periodosTotal);
            nmeses = modelo.sin.numMeses - nmeses;
            nanos = int(nmeses/12);
            nmeses = nmeses % 12;
            data_entrada = modelo.sin.inicioSim + timedelta(days = (nanos*365+nmeses*30));
            saidaResul.write(str(hidro) + ": " + str(data_entrada) + "\n");
    ...
    def imprimeResumoTransmissao(self, aba, linha_inicio):
        ...
        # limpa celulas
        for row in aba['I' + str(pos + 8) + ':L' + str(pos + 37)]:
            for cell in row:
                cell.value = None
        ...
        for icen in (m.condicoes):
            ...
            for isis in (m.subsistemas):
                for iper in (m.periodosTotal):
                    ...
                    saidaResul.write(str(m.enUFVEx[isis,iper,icen] * s.subsistemas[isis].fatorPatUFV[0][iper%12]) + ";");
