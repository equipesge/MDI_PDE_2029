...
class Sistema:
    ...
    def __init__(self, recebe_dados, combSeriesHidroEol):
        ...
        # importa os fatores dos patamares para as usinas
    ...
    def importaDadosGerais(self):
        ...
        self.pldMin = self.fonte_dados.pegaEscalar("C10"); # valor do custo para bombeamento
    ...
    def importaUsinas(self):
        ...
        while (self.fonte_dados.pegaEscalar("A3", lin_offset=iUsina)is not None):
            ...
            if (self.fonte_dados.pegaEscalar("D3",lin_offset=iUsina) == 0):
                ...
                # incrementa o contador de projetos de UHE
                iprojUHE+=1;
        ...
        # percorre a aba Renov Ind. e aloca os projetos de Renovavel em uma lista
        while (self.fonte_dados.pegaEscalar("A3", lin_offset=iUsina)is not None):
            
            # instancia o Projeto e aloca no respectivo subsistema
        ...
        # loop para importar dados das usinas renovaveis existentes e contratadas e alocar nos subsistemas
        ...
        while (self.fonte_dados.pegaEscalar("A3", lin_offset=iUsina)is not None):
            ...
            for iper in range(per_entrada-1,self.numMesesTotal):
                ...
                # incrementa as listas com os montantes de energia e potencia instalada de cada tipo de usina
    ...
    def importafatorPat(self):
        ...
        while (self.fonte_dados.pegaEscalar("A3", lin_offset=linhaOffset)is not None):
            # identifica o subsistema e le os fatores de eolicas indicativas
            subsis = self.subsistemas[int(self.fonte_dados.pegaEscalar('A3', linhaOffset))-1];
            ...
            for patamar in range(0, self.nPatamares):
                ...
                subsis.fatorPatEOL[patamar] = self.fonte_dados.pegaVetor('B3', 'horizontal', 12, linhaOffset, colunaOffset);
            # incrementa o contador de linha e zera o contador de coluna
            linhaOffset += 1;
            colunaOffset = 0;

        # descobre em qual linha comeca as usinas renovaveis EOL existente
        # soma-se o numero 3 porque precisa-se novamente pular as linhas referentes ao cabecalho da tabela, que nao contem informacao relevante para este codigo
        linhaCerta = 'A' + str(linhaOffset + 3 + 3);
        
        # preenche os vetores com os fatores para as renovaveis EOL existente
        offsetAntigo = linhaOffset + 3 + 3;
        linhaOffset = 0;
        while (self.fonte_dados.pegaEscalar(linhaCerta, lin_offset=linhaOffset)is not None):
            # identifica o subsistema e le os fatores de EOL existente
            subsis = self.subsistemas[int(self.fonte_dados.pegaEscalar("A3", linhaOffset))-1];
            ...
            for patamar in range(0, self.nPatamares):
                ...
                subsis.fatorPatEOLEx[patamar] = self.fonte_dados.pegaVetor('B' + str(offsetAntigo), 'horizontal', 12, linhaOffset, colunaOffset);
            # incrementa o contador de linha e zera o contador de coluna
            linhaOffset += 1;
            colunaOffset = 0;

        # descobre em qual linha comeca as usinas renovaveis EOL offshore
        # soma-se o numero 3 porque precisa-se novamente pular as linhas referentes ao cabecalho da tabela, que nao contem informacao relevante para este codigo
        linhaCerta = 'A' + str(offsetAntigo + linhaOffset + 3);
        
        # preenche os vetores com os fatores para as renovaveis EOL offshore
        offsetAntigo = offsetAntigo + linhaOffset + 3;
        linhaOffset = 0;
        while (self.fonte_dados.pegaEscalar(linhaCerta, lin_offset=linhaOffset)is not None):
            # identifica o subsistema e le os fatores de EOL offshore
            subsis = self.subsistemas[int(self.fonte_dados.pegaEscalar("A3", linhaOffset))-1];
            ...
            for patamar in range(0, self.nPatamares):
                ...
                subsis.fatorPatEOF[patamar] = self.fonte_dados.pegaVetor('B' + str(offsetAntigo), 'horizontal', 12, linhaOffset, colunaOffset);
            # incrementa o contador de linha e zera o contador de coluna
            linhaOffset += 1;
            colunaOffset = 0;  
        
        # descobre em qual linha comeca as usinas renovaveis solares
        # soma-se o primeiro 3 porque o loop anterior ja comeca da linha 3, logo essas linhas "puladas" precisam ser acrescentadas a variavel
        # soma-se o segundo 3 porque agora precisa-se pular as linhas referentes ao cabecalho da tabela, que nao contem informacao relevante pra este codigo
        linhaCerta = 'A' + str(linhaOffset + offsetAntigo + 3);
        
        # preenche os vetores com os fatores de ponta para as renovaveis solares
        offsetAntigo = offsetAntigo + linhaOffset + 3;
        linhaOffset = 0;
        while (self.fonte_dados.pegaEscalar(linhaCerta, lin_offset=linhaOffset)is not None):
            # identifica o subsistema e le os fatores de fotovoltaicas
            subsis = self.subsistemas[int(self.fonte_dados.pegaEscalar("A3", linhaOffset))-1];
            ...
            for patamar in range(0, self.nPatamares):
                ...
                subsis.fatorPatUFV[patamar] = self.fonte_dados.pegaVetor('B' + str(offsetAntigo), 'horizontal', 12, linhaOffset, colunaOffset);
            # incrementa o contador de linha e zera o contador de coluna
            linhaOffset += 1;
            colunaOffset = 0;
        
        # descobre em qual linha comeca as usinas renovaveis de biomassa
        # soma-se o numero 3 porque precisa-se novamente pular as linhas referentes ao cabecalho da tabela, que nao contem informacao relevante para este codigo
        linhaCerta = 'A' + str(offsetAntigo + linhaOffset + 3);
        
        # preenche os vetores com os fatores de ponta para as renovaveis de biomassa
        offsetAntigo = offsetAntigo + linhaOffset + 3;
        linhaOffset = 0;
        while (self.fonte_dados.pegaEscalar(linhaCerta, lin_offset=linhaOffset)is not None):
            # identifica o subsistema e le os fatores de biomassa
            subsis = self.subsistemas[int(self.fonte_dados.pegaEscalar("A3", linhaOffset))-1];
            ...
            for patamar in range(0, self.nPatamares):
                ...
                subsis.fatorPatBIO[patamar] = self.fonte_dados.pegaVetor('B' + str(offsetAntigo), 'horizontal', 12, linhaOffset, colunaOffset);
            # incrementa o contador de linha e zera o contador de coluna
            linhaOffset += 1;
            colunaOffset = 0;
        
        # descobre em qual linha comeca as usinas renovaveis PCH
        # soma-se o numero 3 porque precisa-se novamente pular as linhas referentes ao cabecalho da tabela, que nao contem informacao relevante para este codigo
        linhaCerta = 'A' + str(offsetAntigo + linhaOffset + 3);
        ...
        linhaOffset = 0;
        while (self.fonte_dados.pegaEscalar(linhaCerta, lin_offset=linhaOffset)is not None):
            # identifica o subsistema e le os fatores de pch
            subsis = self.subsistemas[int(self.fonte_dados.pegaEscalar("A3", linhaOffset))-1];
            ...
            for patamar in range(0, self.nPatamares):
                ...
                subsis.fatorPatPCH[patamar] = self.fonte_dados.pegaVetor('B' + str(offsetAntigo), 'horizontal', 12, linhaOffset, colunaOffset);
            # incrementa o contador de linha e zera o contador de coluna
            linhaOffset += 1;
            colunaOffset = 0;
    ...
    def criaAgrints(self):
        ...
        while (self.fonte_dados.pegaEscalar("A50", lin_offset = num_agrints) is not None):
            num_agrints += 1;
        ...
        lista = [a.construirLista() for a in self.agrints];
...
class Subsistema:
        ...
        def importaCargaPatamar(self):
            ...
            linhaOffset = 32 + (26*(self.sis_index-1));
